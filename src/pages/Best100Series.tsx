  ...
  useEffect(() => {
    async function fetchTop100Series() {
      try {
        ...
        // Fetch 5 pages to get 100 series (20 per page)
        const promises = [];
        ...
        const top100 = allSeries.slice(0, 100);
        
        setSeries(top100);
      } catch (err) {
        console.error('Error fetching top 100 series:', err);
        setError('Failed to load top series. Please try again later.');
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="text-gray-600 dark:text-gray-400">Loading the best 100 series...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-yellow-500 via-red-500 to-pink-500 p-8 text-white">
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-4xl font-bold">Top 100 Series</h1>
          </div>
          <p className="text-lg opacity-90 max-w-2xl">
            Discover the highest-rated TV series of all time, ranked by viewers worldwide.
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-lg p-4 text-center shadow-md">
          ...
          <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">Top Series</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-lg p-4 text-center shadow-md">
